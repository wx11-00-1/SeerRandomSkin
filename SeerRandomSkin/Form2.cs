    ...
    public partial class Form2 : Form
    {
        ...
        private void Form2_Load(object sender, EventArgs e)
        {
            richTextBox1.Text =
                ...
                "- 可以单独设置某些精灵不随机换肤\n\n" +
                "### 关于皮肤动画的流畅程度问题\n" +
                "- 很多皮肤的动画播放起来会有卡顿的感觉，也有人称之为变速失效\n" +
                "- 缓解方式：\n" +
                "  1. 从随机皮肤列表删掉，或者指定皮肤替换（有可能只是对战其中一方的皮肤导致的卡顿，建议确认一下是哪边的问题）\n" +
                "  2. 皮肤列表只写一个序号，效果就是全体换成同一个皮肤\n" +
                "  3. 地图上人多的话也会导致对战动画卡顿（哪怕是开了屏蔽其他赛尔），换个地图试试\n" +
                "  4. 忍耐一下（皮肤太好看，舍不得删）\n" +
                "  5. 进入宁静祥和的一倍速新世界\n" +
                "\n" +
                "## 自定义窗口尺寸\n" +
                ...
                "\n" +
                "## 变速\n" +
                "- 只会登录器启动时开启一次，可以设置是否自动打开调整倍速的窗口，不打开就一直保持同一个速度\n" +
                "- 在 H5 端长时间游玩后，切换回 Flash 会失效\n" +
                "\n" +
                "## Flash 封包\n" +
                "- 勾选后，登录器启动时会打开另一个窗口，可以查看和发送封包\n" +
                "- 解析封包需要消耗额外的电脑性能，不需要的时候请关闭\n" +
                "\n" +
                "# Flash 逛地图\n" +
                ...
                "- 状态备份" +
                "  1. 背景" +
                "  默认有一些保存、更换状态的脚本，这个状态就是套装、称号、精灵背包的状态；\n" +
                "  其中精灵背包的记录是对应单个米米号的，也就是说，保存过账号的状态，只能由同一个账号更换，其他账户使用的话会报错，如果有多个账号需要自行制作脚本\n" +
                "  2. 备份" +
                "  菜单 -> 开发者工具 -> Application -> Local Storage -> 游戏网址 -> LanBaiState\n" +
                "  保存这个 key 对应的值，在新版本登录器的相同位置创建相同的 key，将保存的值粘贴过去\n" +
                "\n" +
                "# 致谢\n" +
                "## 梦开始的地方\n" +
